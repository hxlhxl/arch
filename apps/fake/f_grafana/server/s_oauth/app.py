...
class Server(object):
    ...
    def _flag(self):
        """
            设置命令行参数解析
        """
        define('port', default=8091, help='server listen port', type=int)
        define('debug', default=False, help='is debug mode', type=bool)
        define('debug_proxy', default=True, help='is request proxy mode', type=bool)
        parse_command_line()
    ...
    def run(self):
        ...
        APPLICATION_SETTINGS.update({
            'debug': options.debug,
            'debug_proxy': options.debug_proxy,
            'debug_proxy_config': self.config.debug_proxy_config,
            # 'autoreload': True,
